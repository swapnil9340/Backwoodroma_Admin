...
export var getBasicGridData = function getBasicGridData(rowLength, colLength) {
  ...
  for (var _j = 1; _j <= pricesColLength; _j += 1) {
    // const y = Math.floor(j / 12);
    columns.push({
      field: "price".concat(_j, "M"),
      headerName: "".concat(_j, "M"),
      type: 'number'
    }); // (y > 0 ? `${y}Y` : '') + `${j - y * 12}M`
  }
